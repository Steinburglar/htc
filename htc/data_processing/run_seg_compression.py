...
class SegCompression(DatasetIteration):
    def __init__(self, paths: list[DataPath], file_type: str, output_dir: Path = None, regenerate: bool = False):
        super().__init__(paths)
        self.file_type = file_type

        if output_dir is None:
            self.output_dir = settings.intermediates_dir_all / "segmentations"
        else:
            self.output_dir = output_dir / "segmentations"
        self.output_dir.mkdir(exist_ok=True, parents=True)

        self.dataset = DatasetImage(self.paths, train=False,)
